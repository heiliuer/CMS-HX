...
function ImageChangeNews(uid, width, height, interval, duration, btitle,
		bsummary, zoomtype, zoomout) {
	...
	_this.onimgload = function(loadIndex) {
		...
		if (loadIndex == _this.currentIndex) {
			_this.constrainimg(document.getElementById(uid + "pic"),
					_this.imgs[loadIndex], document.getElementById(uid
							+ "imgdiv"));
		}
	}
	...
	_this.addimg = function(img, url, title, summary) {
		...
		_this.imgs[imgcount].onload = function() {
			_this.onimgload(imgcount)
		};
	...
	_this.changeimg = function(n) {
		...
		_this.currentTimer = window.setInterval(_this.onchangeimg,
				interval * 1000);// 设置循环周期为4000
	...
	_this.onchangeimg = function() {
		try {
			...
			if (_this.imgs[_this.nextIndex].getAttribute("loadedflag") == null) {
				document.getElementById(uid + "pic").src = "/static/images/28now5fyp0.jpg";
			} else {
				// 图片正常加载, 设置图片
				_this.constrainimg(document.getElementById(uid + "pic"),
						_this.imgs[_this.nextIndex], document
								.getElementById(uid + "imgdiv"));
			}
			// 设置当前图片编号
	...
	_this.constrainimg = function(imgobj, imagevar, imgdiv) {
		try {
			...
			if(widthrate>heightrate){//过宽
				imgWidth=imagevar.width/heightrate;
				imgobj.style.left="-"+(imgWidth-width)/2+"px";
			}else{//过高
				imgHeight=imagevar.height/widthrate;
				imgobj.style.top="-"+(imgHeight-height)/2+"px";
			}
	...
	// 等比例设置图片
	_this.constrainimgBak = function(imgobj, imagevar, imgdiv) {
		try {
			var widthrate = imagevar.width / width;
			var heightrate = imagevar.height / height;
			var imgwidth = 0;
			...
			if (widthrate > 1 || heightrate > 1) // 图片过大时
			{
				if (zoomtype)// 按比例缩小
				{
					...
					imgwidth = Math.max(1, Math.min(imagevar.width / rate,
							width));
					imgheight = Math.max(1, Math.min(imagevar.height / rate,
							height));
				} else// 拉伸
				{
					imgwidth = width;
			...
			} else if (widthrate < 1 || heightrate < 1) // 图片小
			{
				if (zoomout)// 自动放大
				{
					if (zoomtype)// 按比例放大
					{
						...
						imgwidth = Math.max(1, Math.min(imagevar.width / rate,
								width));
						imgheight = Math.max(1, Math.min(
								imagevar.height / rate, height));
					} else // 拉伸
					{
						imgwidth = width;
						imgheight = height;
...
var u_u35_icn = new ImageChangeNews("u_u35_", 1150, 390, 5, 2.0, true, false,
		false, false);
